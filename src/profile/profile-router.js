...
profileRouter.route("/").post(requireAuth, jsonBodyParser, (req, res, next) => {
  ...
  let updateTechList = ProfileService.getAllTech(req.app.get("db")).then(
    (res) => {
      ...
      for (const [key, value] of Object.entries(techs)) {
        if (!results.includes(value)) {
          let newTech = {};
          newTech.name = value;
          ProfileService.insertNewTech(req.app.get("db"), newTech).then(() => {
            return res;
          });
        }
      }
  ...
  Promise.all([updateTechList])
    .then(() => {
      for (const [key, value] of Object.entries(techs)) {
        ProfileService.getTechId(req.app.get("db"), value).then((res) => {
          // insert profile
